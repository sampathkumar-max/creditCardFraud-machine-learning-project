...
class Training_val:
    ...
    def schema_values(self):
        try:
            ...
            self.Log.log("log_msg//schema_values.txt",message)
    ...
    def create_good_bad_folder(self):
        try:
            path=os.path.join("Training_raw_file//Good_data")
            if not os.path.isdir(path):
                os.makedirs(path)
            path=os.path.join("Training_raw_file//bad_data")
            if not os.path.isdir(path):
                os.makedirs(path)
            message = "succesfully  created good and bad raw data folder"
            self.Log.log("log_msg//create_good_bad.txt", message)
    ...
    def delete_good_data(self):
        try:
            path="Training_raw_file//Good_data"
            if os.path.isdir(path):
                shutil.rmtree(path)
            message = "succesfully   deleted good raw data folder"
            self.Log.log("log_msg//delete_good.txt", message)
        except Exception as e:
    ...
    def delete_bad_data(self):
        try:
            path="Training_raw_file//bad_data"
            if os.path.isdir(path):
                shutil.rmtree(path)
            message = "succesfully   deleted bad raw data folder"
            self.Log.log("log_msg//delete_bad.txt", message)
        except Exception as e:
    ...
    def move_bad_to_archivedbad(self):
        try:
            ...
            source="Training_raw_file//bad_data"
            if os.path.isdir(source):
                path="Training_Archived"
                if not os.path.isdir(path):
                    os.makedirs(path)
            ...
            self.Log.log("log_msg//Archived_bad.txt", message)
    ...
    def cheack_filename(self,len_date,lentime):
        try:
            list_files=os.listdir("Training_Batch_Files")
            for file in list_files:
                ...
                if (s_split[0]=="creditCardFraud") and (len(s_split[1])==len_date) and (len(s_split[2])==lentime):
                    shutil.copy("Training_Batch_Files//"+file,"Training_raw_file//Good_data")
                else:
                    shutil.copy("Training_Batch_Files//" + file, "Training_raw_file//bad_data")
    ...
    def cheack_col(self,numofcol):
        try:
            ...
            for file in list_files:
                data=pd.read_csv("Training_raw_file//Good_data//"+file)
                if data.shape[1]==numofcol:
                    pass
                else:
                    shutil.move("Training_raw_file//Good_data//"+file,"Training_raw_file//bad_data")
            message = "succesfully   checked all columns and copyed to the respect folders"
            self.Log.log("log_msg//check_column.txt", message)
            ...
            for file in list_files:
                ...
                for col in data.columns:
                    if len(data[col])-data[col].count()==len(data[col]):
                        shutil.move("Training_raw_file//Good_data//"+file,"Training_raw_file//bad_data")
            message = "succesfully   checked all columns and copyed to the respect folders"
            self.Log.log("log_msg//check_missing_column.txt", message)
        except Exception as e:
